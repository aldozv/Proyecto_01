...
-- CORREGIDO 2026-09-04: gerencia sale de dm_cliente (vigente), no de dm_venta -- ver CLAUDE.md
-- (caso Ucayali Beer, reasignado Huancay Ch -> DA Jun Puc en abril 2026).
-- Nota: sin canal_meta (eso lo cubre Query_CentroOriente_Canal_CD_Categoria_Mensual.sql, que
-- tiene el mismo grano + canal y puede sustituir a esta query sumando sobre canal_meta).
SELECT
  c.gerencia,
...
GROUP BY
  ...
  CASE
    ...
    WHEN v.estratificacion IN ('Gaseosas','Agua','Maltas') THEN 'Nabs'
  END
ORDER BY c.gerencia, anio, mes_num, centro, categoria_grupo
