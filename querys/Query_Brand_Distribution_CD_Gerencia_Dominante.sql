-- Brand Distribution oficial por CD x gerencia (ultimos 13 meses) -- para identificar que
-- gerencia realmente pesa en cada CD por VOLUMEN, a diferencia de dm_cliente.centro_id que solo
-- muestra que algun cliente de esa gerencia esta registrado ahi (incluye clientes reasignados/
-- inactivos con volumen despreciable, ver BITACORA_TABLAS.md).
-- Resultado (confirmado 2026-09-02): casi todos los CD estan dominados por 1 sola gerencia
-- (>98% del volumen) -- ver mapeo "CD dominante por gerencia" en CLAUDE.md.
select
    c.centro_id,
    v.gerencia,
    sum(case
...
and v.fecha_venta between '2025-08-01' and '2026-08-31'
...
group by c.centro_id, v.gerencia
order by c.centro_id, bd desc
