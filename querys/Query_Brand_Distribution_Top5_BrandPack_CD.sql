-- Top 5 marca x pack por CD (ranking por volumen total ene'24-ago'26), con su serie mensual
-- completa. Alimenta las lineas de producto del artifact "Evolución de Brand Distribution" cuando
-- se selecciona un CD (linea Total del CD + top 5 productos de ese CD).
with base as (
    select
        c.centro_id,
        m.brand_pack,
...
totals as (
    select centro_id, brand_pack, sum(bd) as total_bd
    from base group by centro_id, brand_pack
),
ranked as (
    select centro_id, brand_pack, total_bd,
        row_number() over (partition by centro_id order by total_bd desc) as rn
    from totals
)
select b.centro_id, r.rn, b.brand_pack, b.mes, b.bd
from base b
join ranked r on r.centro_id = b.centro_id and r.brand_pack = b.brand_pack
where r.rn <= 5
order by b.centro_id, r.rn, b.mes
