...
SELECT
  CASE WHEN r.`Agrupador (Sku)` IS NULL THEN 'Sin mapear' ELSE 'Mapeado' END AS estado_mapeo,
  COUNT(*) AS filas,
...
GROUP BY CASE WHEN r.`Agrupador (Sku)` IS NULL THEN 'Sin mapear' ELSE 'Mapeado' END
LIMIT 10;
