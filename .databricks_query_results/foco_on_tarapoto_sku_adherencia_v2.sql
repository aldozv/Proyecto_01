WITH gerencia_vigente_cliente AS (
  SELECT cliente_id, max(gerencia) AS gerencia
  FROM slv_maz_dataexperience_peru_dm.dm_venta
  WHERE direccion = 'PE Dir Centro Orient'
    AND mes BETWEEN 202601 AND 202608
    AND agrupador = 'Venta'
    AND indicadores_comerciales = 1
  GROUP BY cliente_id
),
elegibles_by_material AS (
  ...
  INNER JOIN gerencia_vigente_cliente g ON g.cliente_id = a.cliente_id
  WHERE e.listado = 'Foco ON Oriente'
    AND a.mes = 202608
    AND a.estado = 'R'
    AND g.gerencia = 'PE Ger P4 Tarapoto'
